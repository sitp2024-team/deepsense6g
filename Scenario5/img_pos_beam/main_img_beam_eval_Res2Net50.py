...
def main():
    ...
    shutil.copy('./main_img_beam_eval_Res2Net50.py', pwd)
    ...
    class CustomModel(nn.Module):
        def __init__(self, num_classes):
            ...
            # Load a pre-trained Res2Net50 model for image feature extraction
            self.res2net = res2net50_26w_4s(pretrained=True)

            # Remove the fully-connected layer of Res2Net50
            self.res2net = nn.Sequential(*list(self.res2net.children())[:-1])

            # Add a fully-connected layer to process the concatenated output
            self.fc = nn.Linear(2048 + 2, num_classes)  # 2048 is the output feature size of Res2Net50?

        def forward(self, image, position):
            # Extract features from the image using the Res2Net50 model
            features = self.res2net(image)
